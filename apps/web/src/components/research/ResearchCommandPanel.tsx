import { researchExampleChips } from "./researchData";
...
export function ResearchCommandPanel({ command, onChangeCommand, onReset, onRun, runState }: ResearchCommandPanelProps) {
  const busy = ["planning", "searching", "reading"].includes(runState);

  return (
    <section className="border border-borderSoft bg-surfaceWhite p-5">
      <p className="technical-label text-textSecondary">Research Command</p>
      <h2 className="mt-3 text-3xl font-black uppercase leading-none">Paper Discovery Input</h2>
      <textarea
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {researchExampleChips.map((chip) => (
          <button
            ...
            disabled={busy}
            key={chip}
            onClick={() => onChangeCommand(`${command.replace(/\.$/, "")} ${chip}.`.trim())}
            type="button"
          >
            {chip}
          </button>
