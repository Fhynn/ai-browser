...
export function WorkflowPlanPanel({ steps }: { steps: AgentStep[] }) {
  return (
    <section className="border border-borderSoft bg-surfaceWhite p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-black uppercase leading-none">Workflow Steps</h2>
        </div>
        ...
        <AnimatePresence>
          {steps.length > 0 ? (
            ...
            <li className="border border-dashed border-borderSoft bg-surfaceLight p-5 text-sm text-textSecondary">
              Workflow plan appears after the agent starts reading the page.
            </li>
          )}
        </AnimatePresence>
      </ol>
